...
class UpdateDialog(wx.Dialog):
	def __init__(self, parent, url):
		super().__init__(None, title="تنزيل التحديثات")
		self.CentreOnParent()

		panel = wx.Panel(self)
		self.status = wx.TextCtrl(panel, -1, value="في انتظار بدء التحميل...", style=wx.TE_READONLY|wx.TE_MULTILINE|wx.HSCROLL)
		cancelButton = wx.Button(panel, wx.ID_CANCEL, "إيقاف التحميل")
	...
	def errorAction(self):
		wx.MessageBox("لا يمكن تحديث البرنامج في الوقت الحالي", "خطأ", style=wx.ICON_ERROR, parent=self)
		shutil.rmtree(update_path)
		self.Destroy()
	def onChanged(self, event):
		self.progress.SetValue(event.value)
		self.status.SetValue("يتم الآن تنزيل التحديث {}".format(event.value))

	def onFinished(self, event):
		wx.MessageBox("اكتمل تنزيل التحديث بنجاح. يرجى الضغط على موافق للشروع في عملية التثبيت", "نجاح", parent=self)
		try:
			self.status.Value = "جاري تثبيت التحديث"
			path = os.path.join(update_path, event.path)
			subprocess.Popen('"{}" /silent'.format(path), shell=True)
		except:
			wx.MessageBox("حدث خطأ ما عند محاولة فتح ملف التثبيت. فضلًا أعد محاولة التحديث مجددًا, أو تواصل مع المطور للإبلاغ بالمشكلة", "خطأ", style=wx.ICON_ERROR, parent=self)
			self.Destroy()
			return
		sys.exit()
